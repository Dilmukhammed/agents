...
class LLM:
    ...
    async def _generate_structured(self, user_prompt: str) -> PlanResponse:
        try:
            # Note: The 'response_format' parameter expects a Pydantic model and is a feature
            # often provided by libraries like 'instructor'. The standard OpenAI library
            # might not support this directly in all versions or require a specific syntax.
            # This code assumes a compatible environment where this works.
            completion = await self.client.chat.completions.create(
                ...
                # The key is to use the unified PlanResponse model here
            ...
            return PlanResponse(
                status="failure",
                reason=f"Error during API call to {self.provider_name} ({self.model_name}): {e}",
                missing_capabilities=[],
                questions_for_user=["The model failed to generate a valid plan. Could you try rephrasing your request?"]
            )
...
if __name__ == "__main__":
    ...
    # The line below is commented out as it requires a patched version of OpenAI client
    # or a library like 'instructor' to work with `response_model`.
    # asyncio.run(main())
